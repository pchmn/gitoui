...
- `core` models git operations as `Effect`s with **typed errors** (`RepoNotFoundError`, `OperationInProgressError`, `ConflictedEntriesError`…, named for the cause per CONTEXT.md), DI (git client as a service), structured concurrency.
